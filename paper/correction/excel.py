...
#           TODO: add counter here
    ...
    if 0 <= count <= 100:
        count += 1
            # print(ans)
        for x in range(1, 7 * num_of_students_paper, 7):
            for item in range(len(Student)):
                    # print(Student[que_num])
                    if item < 100:
                        sheet.write(1 + 3, item + 1, Student[item])
                        print(item)
                    elif item > 100:
                        sheet.write(5 + 3, item + 1, Student[item])
                        print(item+1)
        if count == 100:
            print(count)
            count = 0
        ...
        #     for item in range(len(que_num)):
        #         print(Student[item])
        #         sheet.write(x + 3, item + 1, Student[item])
...
def single_export_excel(GRADINGLIST, FinelScore, CorrectAns, StudentAns, num_of_students_paper):
    # Create an in-memory output file for the new workbook.
    output = io.BytesIO()
...
#           TODO: add counter here
    ...
    if 0 <= count <= 100:
        ...
        grading = [item for sublist in GRADINGLIST for item in sublist]
        TotelGRADING = [item for sublist in grading for item in sublist]
        print(TotelGRADING)
        GRADING = []
        for i in TotelGRADING:
                if i == 0:
        ...
        for g in range(1, 7 * num_of_students_paper, 7):
            for num in range(len(que_num)):
                sheet.write(g + 4, num + 1, GRADING[num])
    ...
    for score in FinelScore:
        sheet.write("B7", score)
    #
    # Close the workbook before sending the data.
    workbook.close()
